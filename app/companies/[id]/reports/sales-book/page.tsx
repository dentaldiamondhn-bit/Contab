...
import { transformToLibroVentas, groupSalesBookItems, computeSalesBookTotals, formatSalesBookForExcel } from '@/lib/reports/sales-book';
import type { SalesBookItem } from '@/lib/reports/sales-book';
...
export default function SalesBookPage() {
  ...
  const [filteredEntries, setFilteredEntries] = useState<SalesBookEntry[]>([]);
  const [autoItems, setAutoItems] = useState<SalesBookItem[]>([]);
  ...
  const loadSalesBook = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const entriesMapped = list.map((sale: any) => ({
          id: sale.id,
          ...
          cai: sale.cai || '',
          net_value: Number(sale.subtotal) || 0,
          tax_value: Number(sale.tax_amount) || 0,
          total_value: Number(sale.total) || 0,
          sales_type: 'sale',
        }));
  ...
  const loadAutoBook = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformed = transformToLibroVentas(data || []);
        setAutoItems(transformed);
      }
    } catch (error) {
      console.error('Error loading automatic sales book:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const groupedAuto = groupSalesBookItems(autoItems);
  const autoTotals = computeSalesBookTotals(groupedAuto);
  ...
  const handleExportCSV = () => {
    ...
    link.download = `Libro_Ventas_${selectedYear}_${selectedMonth.toString().padStart(2, '0')}.csv`;
    link.click();
  };
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const exportItems: SalesBookItem[] =
        dataSource === 'auto'
          ? autoItems
      ...
      const rows = formatSalesBookForExcel(exportItems);
      ...
      XLSX.utils.book_append_sheet(workbook, ws, 'Libro de Ventas');
      XLSX.writeFile(
        workbook,
        `Libro_Ventas_${companyInfo.name.replace(/\s+/g, '_')}_${selectedYear}-${selectedMonth.toString().padStart(2, '0')}.xlsx`
      );
    } catch (error) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center print:hidden">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleExportCSV} title="Descargar Libro de Ventas en CSV">
            <Download className="w-4 h-4 mr-2" />
            Exportar CSV
          </Button>
          <Button variant="outline" onClick={handleExportExcel} title="Descargar Libro de Ventas en Excel">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              ...
              <div className="text-sm text-gray-500">cuentas de ingreso con balance</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              ...
              <div className="text-sm text-gray-500">ventas + ingresos</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Registro de Ventas - {months.find(m => m.value === selectedMonth)?.label} {selectedYear}
          </CardTitle>
          {dataSource === 'auto' && (
          ...
          {loading ? (
            ...
            autoItems.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                No hay ventas registradas en las transacciones contables para el período seleccionado.
              </div>
            ) : (
                    ...
                    {autoItems.map((item, index) => (
                      <tr key={`${item.code}-${index}`} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-2">
                          <Badge
                            variant={item.type === 'VENTA' ? 'default' : item.type === 'INGRESO' ? 'secondary' : 'outline'}
                          >
                            {item.type}
                          </Badge>
                        </td>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-gray-50">
                      ...
                      <th className="text-left py-3 px-2">RTN Cliente</th>
                      <th className="text-left py-3 px-2">Nombre Cliente</th>
                  ...
                  <tbody>
                    {filteredEntries.map((entry, index) => (
                      <tr key={entry.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-2 font-mono text-xs">{formatRTN(entry.customer_rtn)}</td>
                        <td className="py-3 px-2">{entry.customer_name}</td>
      ...
      <Card className="bg-cyan-50 border-cyan-200 print:hidden">
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <p>
            <strong>ISV Total:</strong> {formatMoney(totals.tax)} - Débito fiscal del período
          </p>
          <p>
            <strong>Total Ventas:</strong> {formatMoney(totals.total)} - Incluye ISV
          </p>
