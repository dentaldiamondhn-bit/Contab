...
-- Paso 9: Eliminar tabla con CASCADE para manejar dependencias
DROP TABLE IF EXISTS tenant_plan_statistics CASCADE;

-- Paso 10: Crear tabla tenant_plan_statistics
...
-- Paso 11: Poblar tenant_plan_statistics con datos existentes
...
-- Paso 12: Verificar y crear tabla Plan si no existe
...
-- Paso 13: Insertar planes básicos si no existen
...
-- Paso 14: Recrear vista tenant_plan_summary con nueva estructura
...
-- Paso 15: Crear función para actualizar estadísticas automáticamente
...
-- Paso 16: Crear trigger para actualizar estadísticas automáticamente
...
-- Paso 17: Crear función para actualizar estadísticas cuando cambia un usuario
...
-- Paso 18: Crear trigger para actualizar estadísticas cuando cambia un usuario
...
SELECT 'Tenant (tabla)' as table_name, COUNT(*) as record_count FROM "Tenant"
UNION ALL
SELECT 'users (tabla)' as table_name, COUNT(*) as record_count FROM users
UNION ALL
SELECT 'Plan (tabla)' as table_name, COUNT(*) as record_count FROM "Plan"
UNION ALL
SELECT 'tenant_plan_statistics (tabla)' as table_name, COUNT(*) as record_count FROM tenant_plan_statistics;

-- Verificación de vistas
SELECT 'invoice_view (vista)' as table_name, COUNT(*) as record_count FROM invoice_view;
SELECT 'invoice_item_view (vista)' as table_name, COUNT(*) as record_count FROM invoice_item_view;
SELECT 'tenant_plan_summary (vista)' as table_name, COUNT(*) as record_count FROM tenant_plan_summary;
SELECT 'Tenants (vista)' as table_name, COUNT(*) as record_count FROM "Tenants";
