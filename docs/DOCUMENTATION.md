...
- **Notas de crédito/débito** fiscales (NC-/ND-)
- **Reportes SAR** y COF
...
### Autenticación (Clerk)
- **Clerk** para manejo de sesiones y RBAC (7 roles, 30+ permisos)
- **Middleware `clerkMiddleware`** protege rutas no públicas con `auth.protect()` → HTTP 404 a requests no autenticados
- **Header `x-tenant-id`** inyectado desde metadata de Clerk
- **Sesiones seguras** por tenant con aislamiento de datos
...
- **Notas de crédito/débito** — Módulo fiscal SAR implementado (`lib/services/notes-service.ts`, `app/api/billing/notes[/[id]]`, `app/billing/notes`, `NoteForm`/`NotePreview`). Numeración NC-/ND- por tenant, asiento contable AJUSTE con ISV 15% (best-effort), estados PENDING/APPLIED/CANCELLED.
- **Middleware con Clerk** — `middleware.ts` usa `clerkMiddleware`; `auth.protect()` devuelve HTTP 404 a requests no autenticados; header `x-tenant-id` inyectado desde metadata de Clerk (rutas públicas listadas en la sección de Seguridad).
- **Fix `/api/companies`** — `process.env.SUPABASE_URL` (inexistente) fue corregido a `process.env.NEXT_PUBLIC_SUPABASE_URL` (GET y PUT).
- **Build** — `pnpm build` EXIT=0 ("Compiled successfully"); errores TS preexistentes que no bloquean: `.next/types/validator.ts`, `app/billing/generate-invoice/page.tsx`, `app/billing/subscriptions/page.tsx`, `lib/billing/invoice-generator.ts`.
- **Consolidación de esquemas** — Migraciones 007 (facturación: `Invoice`/`InvoiceItem`) y 008 (inventario: `product`/`inventory_movement`) aplicadas el 16 Sept 2026; las tablas/vistas legacy fueron eliminadas. Prisma realineado y build en verde.
- **Infraestructura** — Next.js 16.3.5 (Turbopack), `output: 'standalone'`. Las tablas se crean desde el SQL Editor de Supabase (sin acceso DDL directo).
