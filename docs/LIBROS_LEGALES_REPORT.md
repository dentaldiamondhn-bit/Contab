...
- Exportación a Excel (.xlsx)
- Exportación CSV/PDF y vista manual intactas
- Crear asiento manual / automático usa valores completos para `Transaction` (`id` + `functionalAmount` generados en el insert Supabase, montos en centavos): los asientos de retenciones, pólizas manuales (`POST /api/accounting/transactions`) y notas NC/ND se persisten sin fallo (23 Sept 2026)
...
| `lib/services/sar-session.ts` + `app/api/accounting/sar-session/route.ts` + `components/accounting/SARSessionPanel.tsx` | **Login único verificado al portal SAR (Anuales / DET-SAR 221 / DIAT)**: verificaLive de credenciales contra el portal (POST FormData usuario/clave/periodo/formulario ISV DECLARACION_ANUAL + Basic + timeout 15s) antes de conectar; sesión única cifrada AES-256-GCM en `system_settings` clave `sar_session:{empresa}` (vence 24 h, auto-reverificación con credenciales guardadas al vencer); 3 subidores envueltos con `getLiveSARSession`+`isSARSessionValid` (sin conexión → `SESION_NO_VERIFICADA` y NO envían); panel único `SARSessionPanel` montado en las 3 pantallas que bloquea la subida salvo sesión `CONECTADO`; POST persiste sesión + sincroniza credenciales (`sar_config`, `annual_tax_sar_config` preservando metodo/tipoDeclaracion/periodo, `diat_sar_config`); HTTP mapeado: CONECTADO 200 / CREDENCIALES_INVALIDAS 401 / PORTAL_NO_DISPONIBLE 502 / CONFIG_INCOMPLETA 422 (23 Sept 2026) |
...
- Login único verificado al portal SAR (23 Sept 2026): sesión única compartida con DET-SAR 221 y DIAT; las credenciales se validan en vivo contra el portal antes de conectar y las tres subidas exigen sesión `CONECTADO`
