...
1. **`scripts/migrations/PACKAGES_SYSTEM_COMPLETE.sql`** - Estructura de tablas (`Packages`, `PackageProducts`), funciones PostgreSQL, políticas RLS y permisos
2. **`scripts/migrations/UPDATE_PACKAGES_RLS.sql`** - Actualización de políticas RLS de paquetes
...
El script aplica contra Supabase. No hay acceso DDL directo (`getaddrinfo ENOTFOUND`), así que no uses `psql \i` contra `db.<ref>.supabase.co`.

### 1. Aplicar desde el SQL Editor de Supabase (RECOMENDADO)
1. Ir a https://app.supabase.com → tu proyecto → **SQL Editor** → **New query**
2. Copiar y pegar el contenido de `scripts/migrations/PACKAGES_SYSTEM_COMPLETE.sql`
3. Click en **Run**
4. Luego ejecutar `scripts/migrations/UPDATE_PACKAGES_RLS.sql` si aplica

### 2. Alternativa local (solo si tienes una instancia PostgreSQL con acceso)
```bash
# En la consola de Supabase o psql local
\i scripts/migrations/PACKAGES_SYSTEM_COMPLETE.sql
```

### 3. Verificar la creación (desde el SQL Editor)
```sql
-- Verificar tablas
