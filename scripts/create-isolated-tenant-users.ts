import { clerkCreateUser, clerkGetUserList } from '../lib/clerk-api';
...
const tenantsToCreate: TenantData[] = [
  {
    ...
    users: [
      { email: 'admin@dental.hn', firstName: 'Admin', lastName: 'Dental', role: 'ADMIN', password: 'TempDental123!' },
      { email: 'manager@dental.hn', firstName: 'Manager', lastName: 'Dental', role: 'MANAGER', password: 'TempDental123!' },
      { email: 'user@dental.hn', firstName: 'Usuario', lastName: 'Dental', role: 'USER', password: 'TempDental123!' }
    ]
  },
  {
    ...
    users: [
      { email: 'admin@contadora.hn', firstName: 'Admin', lastName: 'Contadora', role: 'ADMIN', password: 'TempConta123!' },
      { email: 'contador@contadora.hn', firstName: 'Contador', lastName: 'Principal', role: 'MANAGER', password: 'TempConta123!' }
    ]
  }
];

async function generateTenantCode(businessName: string): Promise<string> {
  const prefix = businessName.replace(/[^a-zA-Z]/g, '').substring(0, 3).toUpperCase();
  let counter = 1;
...
function getPermissionsForRole(role: string): string[] {
  const permissions: Record<string, string[]> = {
    'ADMIN': ['tenant:admin', 'users:tenant_manage', 'inventory:manage', 'accounting:manage', 'reports:tenant'],
    'MANAGER': ['inventory:view', 'inventory:create', 'inventory:edit', 'accounting:view', 'accounting:create', 'reports:basic'],
    'USER': ['inventory:view', 'accounting:view', 'reports:personal'],
    'VIEWER': ['inventory:readonly', 'accounting:readonly', 'reports:view']
  };
  return permissions[role] || [];
}

function createIsolatedMetadata(role: string, tenantId: string, tenantCode: string): IsolatedUserMetadata {
  return {
    role,
    tenantId,
    tenantCode,
    permissions: getPermissionsForRole(role),
    isolation: { tenantScope: true, crossTenantAccess: false, dataVisibility: 'tenant_only' }
  };
}
  ...
  try {
    ...
    for (const userData of tenantData.users) {
      try {
        ...
        createdUsers.push({ email: userData.email, role: userData.role, clerkId: clerkUser.id, dbId: dbUser.id });
    ...
    return { success: true, tenant: { id: tenant.id, businessName: tenant.businessName, tenantCode: tenant.tenantCode }, users: createdUsers };
  } catch (error: any) {
...
async function checkExistingTenants() {
  try {
    const tenants = await db.tenant.findMany({
      select: { id: true, businessName: true, tenantCode: true, isActive: true, _count: { select: { users: true } } },
...
async function main() {
  ...
  if (successful.length > 0) {
    ...
    successful.forEach(result => {
      ...
      result.users.forEach(user => console.log(`    - ${user.email} (${user.role})`));
    });
  }
}
...
if (command === 'check') {
  ...
  console.log('\nVariables de entorno requeridas: CLERK_SECRET_KEY');
}
