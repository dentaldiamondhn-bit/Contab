-- Crear tablas necesarias para el onboarding (CORREGIDO)
-- Ejecutar en Supabase SQL Editor

-- 1. Crear tabla companies (CORREGIDO: tenant_id como TEXT para coincidir con Tenant.id)
...
-- NOTA: Mantener auth.uid() como UUID para comparar con User.authid (UUID), luego convertir tenantid a TEXT
...
CREATE POLICY "Tenants can view own companies" ON companies
  FOR SELECT USING (tenant_id IN (SELECT tenantid::TEXT FROM "User" WHERE authid = auth.uid()));

CREATE POLICY "Tenants can insert own companies" ON companies
  FOR INSERT WITH CHECK (tenant_id IN (SELECT tenantid::TEXT FROM "User" WHERE authid = auth.uid()));

CREATE POLICY "Tenants can update own companies" ON companies
  FOR UPDATE USING (tenant_id IN (SELECT tenantid::TEXT FROM "User" WHERE authid = auth.uid()))
  WITH CHECK (tenant_id IN (SELECT tenantid::TEXT FROM "User" WHERE authid = auth.uid()));

CREATE POLICY "Tenants can delete own companies" ON companies
  FOR DELETE USING (tenant_id IN (SELECT tenantid::TEXT FROM "User" WHERE authid = auth.uid()));
